...
class TextTokenizer:
    def __init__(self, tokenizer_type, text_data=None, stoi=None, itos=None, known_token="<UNK>"):
    ...
    def _initialize_char_tokenizer(self):
        if self.stoi is not None and self.itos is not None:
            self.encode_func = lambda s: [self.stoi.get(c, self.stoi[self.known_token]) for c in s]
            self.decode_func = lambda l: ''.join([self.itos.get(i, self.known_token) for i in l])
        else:
            chars = sorted(list(set(self.text_data)))
            if self.known_token not in chars:
                chars.append(self.known_token)
    ...
    def _initialize_word_tokenizer(self):
        ...
        self.known_token_idx = list(set(enc.encode(self.known_token)))
        if self.stoi is None and self.itos is None:
            self.text_token_idx= list(set(enc.encode(self.text_data)))
            print("This is the known token idx :", self.text_token_idx[:5])
            print("This is the unknown token idx :",self.known_token_idx)
            self.unique_token_idx = sorted(self.text_token_idx+ self.known_token_idx)
        ...
        def encode(text):
            ...
            return [self.stoi.get(num, self.known_token_idx[0]) for num in encoded_numbers] # self.stoi[-1] SHOULD BE IMPROVED, THIS DOESN'T WORK IF THE ORDER IS ALTERED

        def decode(encoded_numbers):
            decoded_sequence = [self.itos.get(num, self.known_token) for num in encoded_numbers]
    ...
    def decode(self, encoded_numbers):
        print("This is the encoded numbers :", encoded_numbers)
